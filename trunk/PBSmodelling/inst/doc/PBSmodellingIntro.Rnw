...
\author{Jon Schnute, Alex Couture-Beil, Rowan Haigh, and Anisa Egeli}
\maketitle
...
We update the package more frequently than we update the User's Guide. For the latest information, see \code{PBSmodelling-Updates.pdf} in the same directory.
...
\vspace*{2ex} \noindent Schnute, J.T., Couture-Beil, A., Haigh, R., and Egeli, A. 2009. PBS Modelling 2.20: user's guide \red{revised from} \ital{Canadian Technical Report of Fisheries and Aquatic Sciences} \bold{2674}: vi + 170 pp.
