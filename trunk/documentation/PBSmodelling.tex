...
\item[Date]\AsIs{2013-03-27}
\item[Title]\AsIs{GUI Tools Made Easy: Interact with Models, Explore Data, Give Dynamic Presentations}
\item[Author]\AsIs{Jon T. Schnute }\email{schnutej-dfo@shaw.ca}\AsIs{,
Alex Couture-Beil }\email{alex@mofo.ca}\AsIs{,
Rowan Haigh }\email{rowan.haigh@dfo-mpo.gc.ca}\AsIs{, and
A.R. (Rob) Kronlund }\email{allen.kronlund@dfo-mpo.gc.ca}\AsIs{}
\item[Maintainer]\AsIs{Rowan Haigh }\email{rowan.haigh@dfo-mpo.gc.ca}\AsIs{}
\item[Depends]\AsIs{R (>= 2.15.0), methods, tcltk}
\item[Suggests]\AsIs{PBSmapping, PBSddesolve, deSolve, KernSmooth, XML}
...
The R directory '.../library/PBSmodelling/doc' offers a complete user
guide PBSmodelling-UG.pdf. To use this package effectively,
please consult the guide.}
\item[License]\AsIs{GPL (>= 2)}
\item[URL]\AsIs{}\url{http://code.google.com/p/pbs-modelling/,}\AsIs{
}\url{http://code.google.com/p/pbs-tools/}\AsIs{}
\end{description}
...
\end{Examples}
\inputencoding{utf8}
\HeaderA{CCA.qbr}{Data: Sampled Counts of Quillback Rockfish  (Sebastes maliger)}{CCA.qbr}
...
(years). Attributes ``syrs'' and ``cyrs'' 
...
\end{Examples}
\inputencoding{utf8}
\HeaderA{clearAll}{Remove all R Objects From a Specified Environment}{clearAll}
...
\end{SeeAlso}
\inputencoding{utf8}
\HeaderA{clearRcon}{Clear the R Console / Focus on the RGui Window}{clearRcon}
\aliasA{focusRgui}{clearRcon}{focusRgui}
...
Due to parsing complications, the expression \code{act} translates various symbols. \\{}
...
2. Underscore period \samp{\_.} to four back slashes and one period \samp{\bsl{}\bsl{}\bsl{}\bsl{}.}. For example,
\begin{alltt}"poop=strsplit(`some.thing.else`,split=`_.`)"\end{alltt}
...
%
\begin{SeeAlso}\relax
\code{\LinkA{createWin}{createWin}}, \code{\LinkA{evalCall}{evalCall}},
\code{\LinkA{getWinAct}{getWinAct}}
\end{SeeAlso}
%
\begin{Examples}
\begin{ExampleCode}
...
%
\begin{SeeAlso}\relax
\code{\LinkA{doAction}{doAction}}, \code{\LinkA{plotAsp}{plotAsp}}  
\end{SeeAlso}
%
\begin{Examples}
\begin{ExampleCode}
...
\aliasA{findSuffix}{findPrefix}{findSuffix}
...
\end{Examples}
\inputencoding{utf8}
\HeaderA{findProgram}{Locates a program in the PATH environment variable }{findProgram}
\keyword{file}{findProgram}
%
\begin{Description}\relax
...
%
\begin{SeeAlso}\relax
\code{\LinkA{Sys.which}{Sys.which}}
\end{SeeAlso}
%
\begin{Examples}
\begin{ExampleCode}
...
\end{Examples}
\inputencoding{utf8}
\HeaderA{getOptions}{Get and Set User Options}{getOptions}
\aliasA{setOptions}{getOptions}{setOptions}
\keyword{methods}{getOptions}
%
\begin{Description}\relax
...
%
\begin{SeeAlso}\relax
See \code{\LinkA{PBSoptions-class}{PBSoptions.Rdash.class}} for more details and 
an example that uses \pkg{PBSmodelling}'s option management functions.
\end{SeeAlso}
\inputencoding{utf8}
\HeaderA{getOptionsFileName}{Get and Set File Name for Saving and Loading of Options}{getOptionsFileName}
\aliasA{setOptionsFileName}{getOptionsFileName}{setOptionsFileName}
\keyword{methods}{getOptionsFileName}
%
\begin{Description}\relax
...
%
\begin{SeeAlso}\relax
\code{\LinkA{loadOptions}{loadOptions}}, \code{\LinkA{saveOptions}{saveOptions}} \\{}
See \code{\LinkA{PBSoptions-class}{PBSoptions.Rdash.class}} for more details and 
an example using \pkg{PBSmodelling}'s option management functions.
\end{SeeAlso}
\inputencoding{utf8}
\HeaderA{getOptionsPrefix}{Get and Set GUI Prefix of Options Class}{getOptionsPrefix}
\aliasA{setOptionsPrefix}{getOptionsPrefix}{setOptionsPrefix}
\keyword{methods}{getOptionsPrefix}
%
\begin{Description}\relax
...
\end{SeeAlso}
\inputencoding{utf8}
\HeaderA{getPBSoptions}{Retrieve A User Option}{getPBSoptions}
...
\end{SeeAlso}
\inputencoding{utf8}
\HeaderA{getWinAct}{Retrieve the Last Window Action}{getWinAct}
...
\end{Author}
\inputencoding{utf8}
\HeaderA{getWinVal}{Retrieve Widget Values for Use in R Code}{getWinVal}
...
%
\begin{SeeAlso}\relax
\code{\LinkA{exportHistory}{exportHistory}}, \code{\LinkA{initHistory}{initHistory}}
\end{SeeAlso}
\inputencoding{utf8}
...
\item[\code{modename}] name of the radio widgets used to change addHistory\bsl{}'s mode. If \code{NULL}, then the 
...
\end{Author}
\inputencoding{utf8}
\HeaderA{lisp}{List Objects in .PBSmodEnv Workspace}{lisp}
\keyword{environment}{lisp}
%
\begin{Description}\relax
The function \code{lisp} returns a vector of character strings
giving the names of the objects in \code{.PBSmodEnv}. It is only
a wrapper for the base function \code{\LinkA{ls}{ls}}.
\end{Description}
%
\begin{Usage}
\begin{verbatim}
...
See the details section of \code{\LinkA{ls}{ls}}.
...
matching \code{pattern} are returned. See \code{\LinkA{ls}{ls}}
for additional details.
\end{ldescription}
\end{Arguments}
%
\begin{Details}\relax
See the base function \code{\LinkA{ls}{ls}} for details.
\end{Details}
%
\begin{Author}\relax
...
%
\begin{SeeAlso}\relax
\code{\LinkA{ls}{ls}}, \code{\LinkA{tget}{tget}} \\{}
\code{\LinkA{glob2rx}{glob2rx}} for converting wildcard patterns to regular expressions.
\end{SeeAlso}
\inputencoding{utf8}
...
\end{SeeAlso}
\inputencoding{utf8}
\HeaderA{loadOptions}{Save and Load Options to and from Disk}{loadOptions}
\aliasA{saveOptions}{loadOptions}{saveOptions}
\aliasA{saveOptionsAs}{loadOptions}{saveOptionsAs}
\keyword{methods}{loadOptions}
%
\begin{Description}\relax
...
\item[\code{prompt}] logical: if \code{TRUE}, prompt the user to select a file from an interactive GUI. 
...
\end{SeeAlso}
\inputencoding{utf8}
\HeaderA{loadOptionsGUI}{Load and Save Options Values to and from a GUI}{loadOptionsGUI}
\aliasA{saveOptionsGUI}{loadOptionsGUI}{saveOptionsGUI}
\keyword{methods}{loadOptionsGUI}
%
\begin{Description}\relax
These functions are used to move option values to and from a GUI. 
Option values are stored within an R object (as referenced by the \code{option.object}). \\{}
...
\end{SeeAlso}
\inputencoding{utf8}
\HeaderA{lucent}{Convert Solid Colours to Translucence}{lucent}
\keyword{color}{lucent}
%
\begin{Description}\relax
...
\item[\code{a}] alpha transparency value (\code{0} = fully transparent, \code{1} = opaque) 
\end{ldescription}
\end{Arguments}
%
\begin{Details}\relax
...
%
\begin{SeeAlso}\relax
\code{\LinkA{pickCol}{pickCol}}, \code{\LinkA{testCol}{testCol}},
\code{\LinkA{col2rgb}{col2rgb}}, \code{\LinkA{rgb}{rgb}}
\end{SeeAlso}
%
\begin{Examples}
\begin{ExampleCode}
...
\end{Examples}
\inputencoding{utf8}
\HeaderA{openUG}{Open Package User Guide}{openUG}
\keyword{package}{openUG}
\keyword{documentation}{openUG}
%
\begin{Description}\relax
...
%
\begin{SeeAlso}\relax
\code{\LinkA{openFile}{openFile}}, \code{\LinkA{showHelp}{showHelp}}, \code{\LinkA{viewCode}{viewCode}}
\end{SeeAlso}
\inputencoding{utf8}
...
\end{Examples}
\inputencoding{utf8}
\HeaderA{pad0}{Pad Values with Leading Zeroes}{pad0}
\keyword{print}{pad0}
%
\begin{Description}\relax
...
\item[\code{f}] factor of 10 transformation on \code{x} before padding
\end{ldescription}
\end{Arguments}
%
\begin{Details}\relax
...
%
\begin{SeeAlso}\relax
\code{\LinkA{show0}{show0}}, \code{\LinkA{GT0}{GT0}}
\end{SeeAlso}
%
\begin{Examples}
\begin{ExampleCode}
...
\end{Description}
\inputencoding{utf8}
\HeaderA{PBSoptions-class}{S4: Project Options Class}{PBSoptions.Rdash.class}
\aliasA{print,PBSoptions-method}{PBSoptions-class}{print,PBSoptions.Rdash.method}
\aliasA{show,PBSoptions-method}{PBSoptions-class}{show,PBSoptions.Rdash.method}
\keyword{classes}{PBSoptions-class}
\keyword{methods}{PBSoptions-class}
%
\begin{Description}\relax
...
%
\begin{SeeAlso}\relax
\code{\LinkA{getOptions}{getOptions}} for retrieving and modifying user options

\code{\LinkA{getOptionsFileName}{getOptionsFileName}} for retrieving and modifying the default options file name

\code{\LinkA{loadOptions}{loadOptions}} for loading and saving options from and to disk

\code{\LinkA{getOptionsPrefix}{getOptionsPrefix}} for retrieving and modifying the GUI prefix (for custom GUI interfaces)

\code{\LinkA{loadOptionsGUI}{loadOptionsGUI}} for setting GUI values to reflect user options and vice-versa
\end{SeeAlso}
%
\begin{Examples}
\begin{ExampleCode}
...
\item[\code{prettyaxis}] logical: if \code{TRUE}, apply the pretty function to both axes.
...
\end{Examples}
\inputencoding{utf8}
\HeaderA{plotSidebars}{Plot Table as Horizontal Sidebars}{plotSidebars}
\keyword{hplot}{plotSidebars}
%
\begin{Description}\relax
...
\item[\code{...}] additional parameters used by \code{par} and \code{polygon}. 
The user can also pass in two non-formal arguments to control the function: \\{}
\code{lbl} -- labels for the x- and y-axis; \\{}
...
%
\begin{SeeAlso}\relax
\code{\LinkA{plotBubbles}{plotBubbles}}, \code{\LinkA{plotFriedEggs}{plotFriedEggs}},
\code{\LinkA{evalCall}{evalCall}}
\end{SeeAlso}
%
\begin{Examples}
\begin{ExampleCode}
...
\end{Examples}
\inputencoding{utf8}
\HeaderA{presentTalk}{Run a Presentation in R}{presentTalk}
%
\begin{Description}\relax
...
\begin{Arguments}
\begin{ldescription}
\item[\code{talk}] name of file containing XML code (e.g., \code{swisstalk.xml}).
\end{ldescription}
\end{Arguments}
%
\begin{Details}\relax
...
%
\begin{SeeAlso}\relax
See \code{\LinkA{PBStalk-class}{PBStalk.Rdash.class}} for more details on 
...
\keyword{connection}{readPBSoptions}
%
\begin{Description}\relax
...
\end{SeeAlso}
\inputencoding{utf8}
\HeaderA{runExample}{Run a Single GUI Example Included with PBS Modelling}{runExample}
\keyword{utilities}{runExample}
%
\begin{Description}\relax
...
\begin{Arguments}
\begin{ldescription}
\item[\code{ex}] string specifying an example in the \code{pkg} directory \code{examples}. 
\item[\code{pkg}] package with an \code{examples} subdirectory. 
\end{ldescription}
\end{Arguments}
%
\begin{Details}\relax
...
\code{odesolve/PBSddesolve} - \code{FishRes};
...
%
\begin{SeeAlso}\relax
\code{\LinkA{runDemos}{runDemos}}, \code{\LinkA{runExamples}{runExamples}}
\end{SeeAlso}
\inputencoding{utf8}
...
%
\begin{SeeAlso}\relax
\code{\LinkA{runDemos}{runDemos}}, \code{\LinkA{runExample}{runExample}}
\end{SeeAlso}
\inputencoding{utf8}
...
\end{Examples}
\inputencoding{utf8}
\HeaderA{selectDir}{Display Dialogue: Select directory}{selectDir}
\keyword{file}{selectDir}
%
\begin{Description}\relax
...
\item[\code{mustexist}] if logical value is \code{TRUE}, only a existing directory can be selected
...
%
\begin{SeeAlso}\relax
\code{\LinkA{selectFile}{selectFile}}
\end{SeeAlso}
%
\begin{Examples}
\begin{ExampleCode}
...
\end{Examples}
\inputencoding{utf8}
\HeaderA{selectFile}{Display Dialogue: Open or Save File}{selectFile}
\keyword{file}{selectFile}
%
\begin{Description}\relax
...
``Files of type:''.
\item[\code{mode}] string: if \code{"save"} display \bold{Save As} prompt, 
if \code{"open"} display \bold{Open} prompt.
\item[\code{multiple}] if \code{TRUE} the open prompt can select multiple files. This has no
...
%
\begin{SeeAlso}\relax
\code{\LinkA{selectDir}{selectDir}}
\end{SeeAlso}
%
\begin{Examples}
\begin{ExampleCode}
...
%
\begin{Details}\relax
Objects can be placed into the PBS options manager (see \code{\LinkA{PBSoptions-class}{PBSoptions.Rdash.class}}).\\{}
...
\end{Examples}
\inputencoding{utf8}
\HeaderA{setWidgetColor}{Update Widget Color}{setWidgetColor}
\keyword{methods}{setWidgetColor}
%
\begin{Description}\relax
...
Sometimes it is useful to ``fake'' an action. Calling \code{setWinAct} allows 
...
\item[\code{add2int}] if \code{TRUE}, add zeroes to integers after the decimal.
\item[\code{round2n}] if \code{TRUE}, round \code{x} first to \code{n} decimal places.
\end{ldescription}
\end{Arguments}
%
\begin{Value}
...
%
\begin{SeeAlso}\relax
\code{\LinkA{pad0}{pad0}}, \code{\LinkA{GT0}{GT0}}
\end{SeeAlso}
%
\begin{Examples}
\begin{ExampleCode}
...
\end{Author}
\inputencoding{utf8}
\HeaderA{showHelp}{Display HTML Help Pages for Packages in Browser}{showHelp}
...
\code{remote} - character string giving a valid URL for the \code{R\_HOME}
...
\code{\LinkA{viewCode}{viewCode}}, \code{\LinkA{showPacks}{showPacks}}
\end{SeeAlso}
\inputencoding{utf8}
...
\end{SeeAlso}
\inputencoding{utf8}
\HeaderA{talk-class}{S4: Present Talk Classes}{talk.Rdash.class}
\aliasA{break-class}{talk-class}{break.Rdash.class}
\aliasA{code-class}{talk-class}{code.Rdash.class}
\aliasA{file-class}{talk-class}{file.Rdash.class}
\aliasA{section-class}{talk-class}{section.Rdash.class}
\aliasA{text-class}{talk-class}{text.Rdash.class}
\keyword{classes}{talk-class}
%
\begin{Description}\relax
...
\code{talk......}root element that constitutes a talk; \\{}
\code{section...}branch element that defines a section within a talk; \\{}
\code{text......}leaf element that specifies text to be printed on the R console; \\{}
\code{file......}leaf element that specifies files to be opened by the OS; \\{}
\code{code......}leaf element that specifies R code to be executed; \\{}
...
\code{section\_id} & \code{integer}   & specify if section does not immediately follow a talk \\{}
\bold{text}       & & \\{}
...
\end{Section}
%
\begin{Section}{Creating S4 Objects}
Objects can be created by calls of the form:
\begin{alltt}
new("talk", name=name)
new("section",
...
%
\begin{SeeAlso}\relax
\code{\LinkA{presentTalk}{presentTalk}} for presenting a talk in R.

\code{\LinkA{xmlGetAttr}{xmlGetAttr}} for retrieving the value of a named attribute in an XML node.

\code{\LinkA{setClass}{setClass}} for creating a class definition.

\code{\LinkA{PBSoptions-class}{PBSoptions.Rdash.class}} for a complicated S4 class implementation.
\end{SeeAlso}
\inputencoding{utf8}
...
\end{SeeAlso}
\inputencoding{utf8}
\HeaderA{testCol}{Display Colours Available Using a Set of Strings}{testCol}
...
(i) by colour name, \\{}
(ii) by hexadecimal colour code created by \code{rgb()}, or \\{}
...
%
\begin{SeeAlso}\relax
\code{\LinkA{pickCol}{pickCol}}, \code{\LinkA{testAlpha}{testAlpha}}, 
\code{\LinkA{testPch}{testPch}}, \code{\LinkA{palettes}{palettes}}
\end{SeeAlso}
%
\begin{Examples}
\begin{ExampleCode}
...
\item[\code{n}] vector of line type numbers.
\item[\code{...}] additional arguments for function \code{lines}.
\end{ldescription}
\end{Arguments}
%
\begin{Note}\relax
...
%
\begin{SeeAlso}\relax
\code{\LinkA{testLwd}{testLwd}}, \code{\LinkA{testPch}{testPch}}, \code{\LinkA{testCol}{testCol}}
\end{SeeAlso}
\inputencoding{utf8}
...
\end{Examples}
\inputencoding{utf8}
\HeaderA{testPch}{Display Plotting Symbols or Octal Strings}{testPch}
...
\item[\code{octal}] logical: if \code{TRUE}, show octal strings (backslash characters) used in text 
statements (e.g., \code{"30\bsl{}272C"} = 30\eqn{^\circ}{}C).
\item[\code{...}] additional arguments for functions \code{points} or \code{text}.
\end{ldescription}
\end{Arguments}
%
\begin{Author}\relax
...
%
\begin{SeeAlso}\relax
\code{\LinkA{testLty}{testLty}}, \code{\LinkA{testLwd}{testLwd}}, \code{\LinkA{addLabel}{addLabel}}
\end{SeeAlso}
%
\begin{Examples}
\begin{ExampleCode}
...
\end{SeeAlso}
\inputencoding{utf8}
\HeaderA{tget}{Get/Print Objects From or Put Objects Into Temporary Work Environment}{tget}
\aliasA{tcall}{tget}{tcall}
\aliasA{tprint}{tget}{tprint}
\aliasA{tput}{tget}{tput}
\keyword{manip}{tget}
\keyword{environment}{tget}
%
\begin{Description}\relax
...
\item[\code{penv}] parent environment, defaults to \code{parent.frame()} called from within the function.
\item[\code{tenv}] temporary working environment, defaults to \code{.PBSmodEnv}.
\end{ldescription}
\end{Arguments}
%
\begin{Details}\relax
...
\item[\code{scope}] If \code{"L"}, create variables local to the parent
...
\begin{Arguments}
\begin{ldescription}
\item[\code{scope}] either \code{"L"} for the parent frame, \code{"P"} for
...
\item[\code{print.console}] logical: if \code{TRUE}, print the results to the console (default).\\{}
The results are also returned invisibly should the user wish to assign the output to an object.
\item[\code{...}] additional arguments (e.g., \code{replace=TRUE} if specifying \code{random=TRUE}).
\end{ldescription}
\end{Arguments}
%
\begin{Value}
...
%
\begin{SeeAlso}\relax
Generic functions \code{\LinkA{head}{head}} and \code{\LinkA{tail}{tail}} exist in the package \pkg{utils}.
\end{SeeAlso}
\inputencoding{utf8}
...
\item[\code{...}] allows user to specify two additional arguments for \code{output=2}: \\{}
\code{remote} - character string giving a valid URL for the \code{R\_HOME}
directory on a remote location; \\{}
\code{update} - logical: if \code{TRUE}, attempt to update the package
index to reflect the currently available packages. 
...
\code{\LinkA{showHelp}{showHelp}}, \code{\LinkA{view}{view}}
\end{SeeAlso}
\inputencoding{utf8}
...
data (a vector as returned by \code{dim(some\_array)}) and
\code{dimnames} is a vector of length \code{sum(dim(some\_array)+1)}
