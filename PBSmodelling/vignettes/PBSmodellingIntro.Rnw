...
Users interested in building new packages can use \pbsm{} and a simpler enclosed package \code{PBStry} as prototypes.
...
The R directory \code{../library/PBSmodelling/doc} includes a complete User's Guide \code{PBSmodelling-UG.pdf}.
